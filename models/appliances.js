...
module.exports.appOn = function(name, callback){
    Appliance.updateOne({name: name}, {$set: {timeStarted: new Date, appOn: true, appOff: false}}, callback);
}

module.exports.appOff = function(name, callback){
    Appliance.updateOne({name: name}, {$set: {timeEnded: new Date, appOff: true, appOn: false}}, callback);
}